...
from algorithm.ingestion.models import DroneFrame
from algorithm.features.models import PersonFeatures, FrameFeatures
from algorithm.config.settings import FeatureConfig
...
class WCHExtractor:
    ...
    7. L2-normalize for cosine similarity matching - converts each person's visual features into a vector of numbers. It then calculates the cosine of the angle between two vectors. A value close to 1 means the vectors point in nearly the same direction (very similar features), while a value close to 0 means they're dissimilar.
    ...
    def extract_frame(
        self, frame: DroneFrame, detectionSet: DetectionSet
    ) -> FrameFeatures:
        """
    ...
    def _extract_single(
        ...
        # Interpolation is the mathematical method used to calculate these new pixel values because of the resize.
        ...
        wch = np.concatenate(
            [
                upper_hist * self.config.upper_weight,
                lower_hist * self.config.lower_weight,
            ]
        )
    ...
    def _compute_region_histogram(self, region: np.ndarray) -> np.ndarray:
        ...
        # S channel: range [0, 256) S (Saturation) Histogram
        s_hist = np.histogram(
            region[:, :, 1].ravel(),
            bins=self.config.bins_per_channel,
            range=(0, 256),
        ...
        # V channel: range [0, 256) - brightness information
        v_hist = np.histogram(
            region[:, :, 2].ravel(),
            bins=self.config.bins_per_channel,
            range=(0, 256),
    ...
    def cosine_similarity(a: np.ndarray, b: np.ndarray) -> float:
        """simple utility to compute cosine similarity between two WCH vectors"""
