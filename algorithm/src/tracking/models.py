...
class TrackedPerson:
    ...
    frames_since_update: (
        int  # Frames since last matched detection (0 = updated this frame)
    )
...
class TrackingResult:
    ...
    frame_num: int
    tracked_persons: list[TrackedPerson] = field(
        default_factory=list
    )  # Only CONFIRMED tracks
    single_view_persons: list[Person3D] = field(
        default_factory=list
    )  # Pass-through, no IDs
    num_tentative: int = 0
    num_coasting: int = 0
    ...
    # this is for printing an object in a structured way, e.g. when printing a list of TrackingResults
